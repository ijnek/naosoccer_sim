...
TEST(TestSimToNao, TestImu)
{
  ...
  imu.angular_velocity.y = 0.5;
  imu.angular_velocity.z = 0.6;

  nao_sensor_msgs::msg::Accelerometer convertedAccelerometer =
    rcss3d_nao::sim_to_nao::getAccelerometer(imu);
  EXPECT_NEAR(convertedAccelerometer.x, 0.1, 0.01);
  EXPECT_NEAR(convertedAccelerometer.y, 0.2, 0.01);
  EXPECT_NEAR(convertedAccelerometer.z, 0.3, 0.01);

  nao_sensor_msgs::msg::Gyroscope convertedGyroscope = rcss3d_nao::sim_to_nao::getGyroscope(imu);
  EXPECT_NEAR(convertedGyroscope.x, 0.4, 0.01);
  EXPECT_NEAR(convertedGyroscope.y, 0.5, 0.01);
  EXPECT_NEAR(convertedGyroscope.z, 0.6, 0.01);
}
